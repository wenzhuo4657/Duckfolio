...
export async function POST(req: NextRequest) {
  ...
  const primaryModelId = ai.primaryModelId(model);
  const selectionModelId = ai.isOpenAICompatible
    ? primaryModelId
      ...
      execute: async ({ writer }) => {
        ...
        if (!toolName) {
          ...
          const enumOptions = isSelecting
            ...
            : ['generate', 'comment'];
          const modelId = model || selectionModelId;
        ...
        const stream = streamText({
          ...
          model: ai.model(primaryModelId),
          // Not used
          prompt: '',
          tools: {
            comment: getCommentTool(editor, {
              messagesRaw,
              model: ai.model(selectionModelId),
              writer,
            }),
          ...
          prepareStep: async (step) => {
            ...
            if (toolName === 'edit') {
              ...
              return {
                ...
                model:
                  editType === 'selection'
                    ? // The selection task is more challenging, so Gateway uses Gemini when available.
                      ai.model(selectionModelId)
                    : ai.model(primaryModelId),
                messages: [
                  {
                    content: editPrompt,
                    role: 'user',
